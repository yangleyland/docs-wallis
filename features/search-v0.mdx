...
Firecrawl integrates its search API with its robust scraping infrastructure to provide a seamless search and scrape functionality through a single endpoint. Here's why:

1. **Unified Search Query:**
   Users submit a search query via the search endpoint.
